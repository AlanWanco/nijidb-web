...
打开 `/programs` 查看节目播出日历；登录后打开 `/admin/programs` 手动维护节目资料和排期。排期支持周更、固定月更、逐期设置、单次和多个分段时期，每个时期可以选择更新时间时区及“自动生成后续单集”开关。逐期设置默认关闭自动生成；开启后按月生成每月 1 日作为占位，之后可在单集列表中直接修改每期原定日期和时间。单次时期默认关闭，开启时只生成时期开始日的一期。对异常节目的单集列表可以关闭后续自动生成，改用“添加单集”逐条录入，也可以将固定月更批量切换为逐期设置，或将已改期日期和时间覆盖为新的原定播出日期和时间。每期可以在备注后上传节目返图（配置 R2 时同步到 R2）或保存图片直链，日历侧栏、节目详情和列表均显示缩略图并支持点击查看大图。一个主节目下可以挂载多个独立配置的子节目，主节目 key 固定为“主节目”。开启自动生成的进行中节目会按规则生成未来约半年的单集，也可以对单集进行改期、取消或补录。“未更新”仅按当前排期规则推算，不代表真实播出状态。
