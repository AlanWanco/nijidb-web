...
打开 `http://localhost:8000`。设置页默认开启音乐自动检查，每 10 分钟检查一次目录、每 5 分钟检查一次异步详情，两个间隔都可以分别调整；关闭自动检查后仍可手动执行“立即检查”。OneBot 的地址、Token 和目标写入管理员设置页。首次抓取仅初始化数据库，之后新增或发生变化的条目才会通知。
