...
节目编辑页支持单节目 JSON 导入和导出。导出内容包括节目资料、多个排期时期、已保存的单集资料、直播/录播、普通/EX、嘉宾和链接；未单独保存的自动生成单集会由排期时期重新生成。导入会先显示节目、排期和全部单集预览，确认后新建节目，不覆盖当前节目。格式模板见 [`docs/program-json-template.json`](docs/program-json-template.json)；说明字段使用合法 JSON 的 `_field_notes` 和 `_import_notes`，也兼容 `//`、`/* */` 注释。导入暂不处理改期、延期和删除状态。
