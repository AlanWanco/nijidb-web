...
- `/collabo`：每次联动一张卡片，单张封面、日期、标题、合作方、图片数；24 项分页，搜索、年份筛选。
- `/collabo/:slug`：左侧完整立绘画廊、右侧资料和链接；主图选择条、图片说明、原图入口、放大对话框。
...
- `/admin/collabo`：复用现有管理员登录，提供记录列表及审核入口。
- `/admin/collabo/new`、`/admin/collabo/:id`：资料、日期含义、合作方、版权、备注、来源链接标题与 URL、多图编辑。
- 图片可以排序、选择封面、补充说明/来源、标记待审核/通过/排除；联动审核通过需要所有候选图已处理且封面已通过。
...
`GET /api/collabo?q=&year=&page=1&page_size=24`
...
- 列表只需要摘要和封面/缩略图地址，不应返回每条记录全部大图。
- 推荐按日期倒序、相同日期按固定 ID 排序；total 是当前筛选后的总数，years 是完整可用年份。

`GET /api/collabo/:slug?q=&year=`
...
- 图片 `review_status`：`pending` / `approved` / `rejected`。公开端仅返回符合发布策略的内容。
- 审核状态是人工决定，图片数量不代表完成度。
- 原 Wiki 完整数据/快照、原始 ID 与采集 provenance 仍由数据库保留；前端不覆盖这些原始字段。

### 管理接口（必须服务端鉴权）

- `GET /api/admin/collabo`：同列表参数与响应，但包含待审核项。
- `GET /api/admin/collabo/:id`：同详情响应，包含排除图片，队列相邻项用数据库 ID。
...
- 本次没有生成任何批量缩略图，也没有上传、压缩或覆盖采集原图。
- 等用户确认本批全部审核完毕，再启动后端的批量任务。
- 推荐封面多尺寸 WebP（如 360/720px），保留完整立绘、不要默认裁掉头脚；原图透明通道保留。
- 原图与派生图使用不同 R2 key；派生图携带内容哈希/版本，上传验证后原子更新数据库引用。
- 支持 dry-run、断点续跑、失败重试、修改封面后的增量更新；不要在用户浏览页面时现算。
- 现有前端已优先读取 `thumbnail_url`，无缩略图时回退到封面；一览分页且每卡仅加载一图，图片 lazy loading。
- 当前客户端仅使用一个 `thumbnail_url`；后端产出多尺寸后再补 `srcset/sizes` 字段与展示，不伪称已完成。
