...
<template>
  <main class="page">
    <div class="detail-topline">
     ...
     <p v-if="loading" class="state">{{ t("正在读取专辑……") }}</p>
    <p v-else-if="error" class="state error">{{ error }}</p>
    <template v-else-if="release">
       <nav class="release-switch" :aria-label="t('切换专辑')">
        <RouterLink v-if="data.previous" :to="`/release/${data.previous.id}`">
           <span class="switch-arrow">←</span><small>{{ t("上一张") }}</small><strong>{{ data.previous.title }}</strong>
        </RouterLink>
         <span v-else class="switch-button disabled"><span class="switch-arrow">←</span><small>{{ t("已经是第一张") }}</small></span>
        <RouterLink v-if="data.following" :to="`/release/${data.following.id}`">
           <span class="switch-arrow">→</span><small>{{ t("下一张") }}</small><strong>{{ data.following.title }}</strong>
        </RouterLink>
         <span v-else class="switch-button disabled"><span class="switch-arrow">→</span><small>{{ t("已经是最后一张") }}</small></span>
      </nav>
      ...
      <article class="release">
        ...
        <div class="release-info">
          ...
          <dl>
             <dt>【アーティスト】</dt><dd>{{ release.artist || t("官方暂未公开") }}</dd>
             <dt>【発売日】</dt><dd>{{ release.release_date || t("官方暂未公开") }}</dd>
             <dt>【価格】</dt><dd>{{ release.price || t("官方暂未公开") }}</dd>
            <template v-for="[key, value] in specs" :key="key">
              <dt>【{{ key }}】</dt><dd>{{ value }}</dd>
            </template>
          </dl>
           <a class="source" :href="release.source_url" target="_blank" rel="noopener noreferrer">{{ t("查看官方原页 ↗") }}</a>
        </div>
      </article>

      <section class="tracks">
         <div class="section-heading"><div><p class="eyebrow">TRACKLIST</p><h2>{{ t("收录内容") }}</h2></div><span class="section-count">{{ tracks.length }} TRACKS</span></div>
        <div v-for="track in tracks" :key="`${track.disc}-${track.number}-${track.title}`" class="track-row">
         ...
         <p v-if="!tracks.length" class="muted">{{ t("官方目前只公开了封面和标题，曲目详情尚未发布。") }}</p>
      </section>

      <section v-if="extras.length" class="extras">
         <div class="section-heading"><div><p class="eyebrow">BONUS / LINKS</p><h2>{{ t("特典与相关链接") }}</h2></div><span class="section-count">{{ extras.length }} ITEMS</span></div>
        <div class="extra-grid">
          <article v-for="(extra, index) in extras" :key="`${extra.title}-${index}`" class="extra-panel">
            <h3>{{ extra.title }}</h3>
             <a v-if="extra.type === 'link'" class="extra-link" :href="extra.url" target="_blank" rel="noopener noreferrer">{{ t("打开链接 ↗") }}</a>
      ...
      <details v-if="release.detail_html" class="raw">
         <summary>{{ t("查看原始规格数据（默认折叠，用于核对）") }}</summary>
