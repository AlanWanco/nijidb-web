...
2. 新闻轮询每趟读该文件，只考虑最近一次探测为 200 且本机没有失败记录的节点，优先避开上一趟的节点和主要出口地区，再随机挑一个，用它的端口作为本趟代理；节点名称没有标准化地区字段，因此地区按名称中的主要地区识别，括号里的“香港中转”“台湾中转”等不会覆盖主要地区；如果没有其他健康地区，则退回同地区的其他健康节点，优先保证可用性；
...
- 轮询保持串行、低频、固定 UA，不使用 Cookie、并发或 UA 轮换；每趟内保持单一出口，跨趟切换时优先避开上一趟的地区。
